...
**Scope:** Last 7 days of commits (window: 2026-06-15 to 2026-06-22)
...
identified during this scan. The `main` branch of the publication repository
received **zero new commits** in the 7-day scan window
(2026-06-15 to 2026-06-22). The current `main` HEAD
(`a6b9118 Merge pull request #2591 from EffortlessMetrics/publish/swarm-ghcr-support-status`,
dated 2026-06-12) sits outside the window and was already fully covered by the
prior weekly scan on 2026-06-15
(`.factory/security/reports/security-report-2026-06-15.md`, branch
`droid/security-report-2026-06-15`). The codebase continues to demonstrate a
...
(`reason = "transitive via tokei; revisit when upstream removes it"`).
...
- `parseGitHubRepo` enforces host allowlisting to `github.com` /
  `www.github.com` and segment pattern `^[A-Za-z0-9_.-]+$` (see
  `web/runner/ingest.js:559`, `GITHUB_SEGMENT_PATTERN`).
...
| D-02 | `unwrap_used`, `expect_used`, `panic`, `unreachable` lints denied | `Cargo.toml` (lines 70-71) | ✓ |
...
| D-22 | GitHub repo segment validation (`^[A-Za-z0-9_.-]+$`) | `web/runner/ingest.js::GITHUB_SEGMENT_PATTERN` | ✓ |
...
- **Last Modified (content):** 2026-06-01 (21 days ago — well within 90-day window)
- **Methodology:** STRIDE
- **Next review:** 2026-09-01 (90-day cadence) or upon architecture change

### Scan Metadata

- **Scan Window:** 2026-06-15 to 2026-06-22 (7 days)
- **Commits Scanned on `main`:** 0
- **`main` HEAD (out of window):** `a6b9118 Merge pull request #2591 from EffortlessMetrics/publish/swarm-ghcr-support-status` (2026-06-12)
- **Files in scope:** 2470 (entire repository at `main` HEAD; covered by 2026-06-15 weekly scan)
- **Scan Duration:** ~3m
- **Skills Used:** threat-model-generation (re-used, current), commit-security-scan (manual), vulnerability-validation (manual), security-review (manual)
- **Manual Reviewers:** 1 (Droid scheduled security scan)
- **False Positive Filter:** applied — see Observations above
- **Workflow:** `.github/workflows/droid-security-scan.yml` (cron `0 8 * * 1`)
...
| Subprocess invocation | `crates/tokmd-git/`, `crates/tokmd-cockpit/src/supply_chain.rs`, `crates/tokmd/src/git_support.rs`, `crates/tokmd-scan/src/walk/git.rs` | 0 |
...
| Web runner (browser) | `web/runner/main.js`, `worker.js`, `auth.js`, `messages.js`, `runtime.js`, `ingest.js`, `ingest.test.mjs` | 0 |
| Container | `Dockerfile`, `.dockerignore` | 0 |
| Threat model | `.factory/threat-model/threat-model.md` | unchanged (21 days old, within 90-day window) |

### Commit-level Analysis

**Zero commits on `main` in the 7-day scan window.**

```
$ git log main --since="2026-06-15" --until="2026-06-22" --oneline
(empty)
```

The current `main` HEAD is `a6b9118`, dated 2026-06-12 — three days *outside*
the scan window. That commit was already covered in depth by the prior weekly
scan (`droid/security-report-2026-06-15`,
`.factory/security/reports/security-report-2026-06-15.md`), which reviewed
the full publication-side re-import with 0 findings at or above the `medium`
threshold.

For traceability, the `main` HEAD commit is:
...
**Why this commit is not re-scanned here:** it lies outside the 7-day window
(`git log main --since="7 days ago"` returns empty), and re-running the same
review would not change the result. Per the dual-repo workbench boundary
(`AGENTS.md` and `docs/ci/swarm-routing.md`), `EffortlessMetrics/tokmd` is
the publication repository and only receives merge-commit imports from
`tokmd-swarm/main`. No swarm feature work happens on `main` here.

**Note on `git log --all` activity:** The remote-tracking refs visible via
`git log --all` show 70+ commits dated 2026-06-15 through 2026-06-22 across
many `remotes/origin/*` branches (sentinel, fuzzer, bridge, gatekeeper,
librarian, auditor, surveyor, steward, etc.). These are swarm-workbench
branches, not part of `main`'s reachable history, and are out of scope for
this publication-side scan. Their security review is handled by the
`droid-security-scan` workflow on `EffortlessMetrics/tokmd-swarm` (the
workbench repository), which has its own threat-model coverage.

**No security findings in this scan.**
...
The next scheduled security scan runs Monday, 2026-06-29 via
...
- Previous scans: `.factory/security/reports/security-report-2026-06-01.md`, `.factory/security/reports/security-report-2026-06-08.md`, and (on branch `droid/security-report-2026-06-15`) `.factory/security/reports/security-report-2026-06-15.md`
