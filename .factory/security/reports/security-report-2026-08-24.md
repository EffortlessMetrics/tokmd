...
**Scope:** Last 7 days of commits (2026-08-17 → 2026-08-24)
...
identified during this scan. The 7-day window (2026-08-17 → 2026-08-24) contains
**zero commits** on `main` in this branch (`git log --since="7 days ago" --pretty=oneline`
returns no output). The most recent commit on `main` is
`ff05889 import: finalize stable release after consumer proof` from 2026-08-05 —
19 days before this scan and already covered by no prior weekly scan (the prior
scan, 2026-07-27, also had a zero-commit window; the last scan that reviewed
non-empty commit content was 2026-07-20). Because no new code or configuration
entered the branch during this scan window, the codebase was reviewed against
the existing `.factory/threat-model/threat-model.md` (per content "Last Reviewed:
2026-08-02", 22 days ago — well within the 90-day freshness window) and all
twenty-three standing defenses (D-01 through D-23) were re-verified by direct
inspection of the source paths called out in the threat model and the previous
report. The codebase continues to demonstrate a security-first design with no
regressions.
...
inspection at the cited locations. All remain intact.

| ID | Defense | Location | Verified |
|----|---------|----------|----------|
| D-01 | `unsafe_code = "forbid"` workspace lint | `Cargo.toml:56` | ✓ |
| D-02 | `unwrap_used`, `expect_used`, `panic`, `unreachable`, `dbg_macro`, `todo`, `unimplemented` lints denied | `Cargo.toml:65-71` | ✓ |
| D-03 | Git subprocess env isolation (`GIT_REPO_SHAPING_ENV`) | `crates/tokmd-git/src/command.rs:9,35,54`, `crates/tokmd/src/git_support.rs:3,25` | ✓ |
| D-04 | Git ref validation (`env_base_ref_is_safe` + `--end-of-options`) | `crates/tokmd-git/src/refs.rs:13,46,54,80,219` | ✓ |
| D-05 | Bounded path canonicalization under root | `crates/tokmd-scan/src/path/bounded_path.rs` | ✓ (file present; no diff this scan) |
| D-06 | FFI in-memory input path validation | `crates/tokmd-core/src/ffi/inputs.rs:12,66,106,116` (`MAX_IN_MEMORY_INPUT_PATH_BYTES = 4096`) | ✓ |
| D-07 | Strict JSON parsing with type validation | `crates/tokmd-core/src/ffi/parse.rs` | ✓ (file present; no diff this scan) |
| D-08 | Per-family schema versioning (`SCHEMA_VERSION=2`, `COCKPIT_SCHEMA_VERSION=3`, `HANDOFF_SCHEMA_VERSION=5`, `CONTEXT_SCHEMA_VERSION=4`, `CONTEXT_BUNDLE_SCHEMA_VERSION=2`) | `crates/tokmd-types/src/lib.rs:79`, `crates/tokmd-types/src/cockpit.rs:19`, `crates/tokmd-types/src/context.rs:16,23,30` | ✓ |
| D-09 | SHA-pinned Droid-related actions; tag-pinned first-party actions | `.github/workflows/droid.yml:59,83` (SHA-pinned `EffortlessMetrics/droid-action-safe@7c1377cc...`) | ✓ |
| D-10 | Branch protection on `main` (CODEOWNERS, 1 approval, CI required) | `.github/settings.yml` | ✓ (file present; no diff this scan) |
| D-11 | `cargo-deny` advisory + license allowlist | `deny.toml` | ✓ (file present; no diff this scan) |
| D-12 | BLAKE3 redaction with extension allowlist | `crates/tokmd-format/src/redact/mod.rs`, `crates/tokmd-format/src/redact/extensions.rs` | ✓ (files present; no diff this scan) |
| D-13 | Content reads bounded by `ContentLimits` (`DEFAULT_MAX_FILE_BYTES = 128 KiB`) | `crates/tokmd-analysis/src/content/mod.rs:17,27,35,42,88` | ✓ |
| D-14 | PyO3 FFI invariants (no panic, GIL release, error translation) | `crates/tokmd-python/src/lib.rs` | ✓ (file present; no diff this scan) |
| D-15 | WASM uses `MemFs` (no host fs) | `crates/tokmd-wasm/` | ✓ (no diff this scan) |
| D-16 | `web/runner` browser runner uses `textContent` (no `innerHTML`/`eval`/`new Function`/`document.write`) | `web/runner/main.js` | ✓ (file present; no diff this scan) |
| D-17 | `web/runner` token stored in `sessionStorage` (not `localStorage`) | `web/runner/auth.js` (tests: `auth.test.mjs:65`) | ✓ |
| D-18 | `web/runner` worker protocol allowlists modes & presets | `web/runner/messages.js` | ✓ (file present; no diff this scan) |
| D-19 | Composite action installs tokmd with sha256 checksum verification | `action.yml` | ✓ (file present; no diff this scan) |
| D-20 | Custom Droid action SHA-pinned across all Droid workflows | `.github/workflows/droid*.yml` | ✓ |
| D-21 | `cargo audit` invoked with structured `--json` output, malformed JSON treated as Pending | `crates/tokmd-cockpit/src/supply_chain.rs:17,47,92,174,188` | ✓ |
| D-22 | `run_json` top-level JSON must be an object (strict shape check) | `crates/tokmd-core/src/ffi/mod.rs::run_json_inner` | ✓ (file present; no diff this scan) |
...
- **Status:** Current (verified unchanged since 2026-08-02 per content header)
- **Location:** `.factory/threat-model/threat-model.md`
- **Per content "Last Reviewed":** 2026-08-02 (22 days ago — well within
  90-day window)
- **Per `git log`:** the threat model was introduced by commit
  `ff05889 import: finalize stable release after consumer proof` on
  2026-08-05; no subsequent edits recorded in git history.
- **Methodology:** STRIDE
...
- **Commits Scanned:** 0 (the 7-day window `git log --since="7 days ago"`
  ...
  2026-08-05, 19 days ago)
- **Files in scope:** Not applicable — no commit diff. The comprehensive
  baseline established by the 2026-06-29 scan (2579 files via the true-merge
  initial import) remains the last fully reviewed delta and is unchanged
  since.
...
baseline (last fully scanned 2026-06-29 and unchanged since), with this
week's spot-checks of standing-defense code paths.
...
The 7-day window (2026-08-17 → 2026-08-24) contains no commits on `main`
in this repository. The branch root for this period is
`ff0588903142cedda2dbc4903bbb7128fa1cbb3e` dated 2026-08-05 01:44:26 -0400,
which is **19 days** before this scan:
...
The `git log --since="14 days ago"` and `git log --since="21 days ago"`
queries also return zero results, confirming a sustained quiet period on
this branch since the 2026-08-05 import commit. No commit-level analysis
