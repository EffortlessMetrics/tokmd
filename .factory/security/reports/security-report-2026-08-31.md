...
**Scope:** Last 7 days of commits (2026-08-24 → 2026-08-31)
...
identified during this scan. The 7-day window (2026-08-24 → 2026-08-31) contains
**zero commits** on `main` in this repository. `git log --since="7 days ago"
--pretty=oneline` returns no output; `git log --since="30 days ago" --pretty=oneline`
returns only the single root commit `ff05889 import: finalize stable release after
consumer proof` from 2026-08-05 (26 days ago). Because no new code or configuration
entered the branch during the scan window, the codebase was reviewed against the
existing `.factory/threat-model/threat-model.md` (last modified 2026-08-02, still
within the 90-day freshness window) and all twenty-three standing defenses (D-01
...
These items were considered during the scan but do not at or above the
`medium` severity threshold. They are recorded here for traceability and the
next scheduled scan. All "carried" observations are unchanged from the
2026-07-27 baseline; no new informational observations were introduced this
week.
...
Other workflows pin by tag (e.g., `actions/checkout@v7.0.0`,
`Swatinem/rust-cache@v2`, `dtolnay/rust-toolchain@stable`). The threat model
claims SHA pinning workspace-wide, which is no longer strictly accurate for
non-Droid workflows.
...
|----|---------|----------|----------|
| D-01 | `unsafe_code = "forbid"` workspace lint | `Cargo.toml` (workspace lint section) | ✓ |
| D-02 | `unwrap_used`, `expect_used`, `panic`, `unreachable`, `dbg_macro`, `todo`, `unimplemented`, `panic_in_result_fn`, `missing_panics_doc`, `should_panic_without_expect` lints denied | `Cargo.toml` (workspace lint section) | ✓ |
| D-03 | Git subprocess env isolation (`GIT_REPO_SHAPING_ENV`) | `crates/tokmd-git/src/command.rs` (line 9: `const GIT_REPO_SHAPING_ENV`) | ✓ |
| D-04 | Git ref validation (`env_base_ref_is_safe` + `--end-of-options`) | `crates/tokmd-git/src/refs.rs` (line 13, 80) | ✓ |
| D-05 | Bounded path canonicalization under root | `crates/tokmd-scan/src/path/bounded_path.rs` (line 14, 21, 34, 108) | ✓ |
...
| D-08 | Per-family schema versioning (`SCHEMA_VERSION=2`, `COCKPIT_SCHEMA_VERSION=3`, `HANDOFF_SCHEMA_VERSION=5`, `CONTEXT_SCHEMA_VERSION=4`, `CONTEXT_BUNDLE_SCHEMA_VERSION=2`); sync tests pass | `crates/tokmd-types/src/lib.rs`, `cockpit.rs`, `context.rs`; `crates/tokmd-types/tests/schema_doc_sync.rs` (2 passed) | ✓ |
...
| D-11 | `cargo-deny` advisory + license allowlist | `deny.toml` (line 15-16: `RUSTSEC-2020-0163` documented) | ✓ |
| D-12 | BLAKE3 redaction with extension allowlist | `crates/tokmd-format/src/redact/mod.rs` (line 91: `blake3::hash`), `extensions.rs` | ✓ |
| D-13 | Content reads bounded by `ContentLimits` | `crates/tokmd-analysis/src/content/mod.rs` (line 17: `DEFAULT_MAX_FILE_BYTES = 128 * 1024`) | ✓ |
...
| D-16 | `web/runner` browser runner uses `textContent` (no `innerHTML`/`eval`/`new Function`/`document.write`) | `web/runner/main.js`; repo-wide grep returns no matches | ✓ |
...
| D-19 | Composite action installs tokmd with sha256 checksum verification | `action.yml` (line 258-279) | ✓ |
| D-20 | Custom Droid action SHA-pinned across all Droid workflows | `.github/workflows/droid*.yml` (`EffortlessMetrics/droid-action-safe@7c1377cc...`) | ✓ |
| D-21 | `cargo audit` invoked with structured `--json` output, malformed JSON treated as Pending | `crates/tokmd-cockpit/src/supply_chain.rs` (line 73, 134, 181, 247) | ✓ |
| D-22 | `run_json` top-level JSON must be an object (strict shape check) | `crates/tokmd-core/src/ffi/mod.rs::run_json_inner` (line 78-80) | ✓ |
| D-23 | Author DAG import via true-merge commits (no force-push of publication history) | repository topology (root commit `ff05889` is a history-preserving import) | ✓ |
...
- **Commits Scanned:** 0 (the 7-day window `git log --since="7 days ago"`
  ...
  2026-08-05, 26 days ago)
- **Files in scope:** Not applicable — no commit diff. The comprehensive
  baseline of 2589 files was last reviewed in the 2026-06-29 weekly scan
  against the true-merge initial import that established the branch root
  on the swarm repo, and again on 2026-07-27 against the steward docs
  manifest fix merge. The current branch root `ff05889` was itself
  published as the import of that reviewed swarm state into the
  publication repo.
- **Scan Duration:** ~4m (focused baseline re-verification, no diff
  resolution required)
...
- **Validation Run:** `cargo test -p tokmd-types --test schema_doc_sync`
  passed (2/2), confirming documented schema versions match code.
...
baseline (last fully scanned 2026-06-29 on swarm and unchanged in
substance since).
...
The 7-day window (2026-08-24 → 2026-08-31) contains no commits on `main`
in this repository. The branch root is the single commit
`ff05889 import: finalize stable release after consumer proof`, dated
2026-08-05 01:44:26 -0400, which is **26 days** before this scan:
...
Subject: import: finalize stable release after consumer proof
    History-preserving import of the reviewed 1.15.1 stable-release ordering fix. Preserve the two-parent publication topology.
```

The commit's annotated message documents it as a history-preserving import
of the 1.15.1 stable-release ordering fix, with the two-parent publication
topology preserved. The underlying workspace content (2589 files) was
already comprehensively reviewed in the 2026-06-29 weekly scan against the
true-merge initial import and re-verified each subsequent week; the
2026-07-27 weekly scan confirmed the steward docs manifest fix merge in
the swarm branch that this import consolidates into the publication
repository. No additional diff review is required for this scan.
