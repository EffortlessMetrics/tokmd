...
**Scope:** Last 7 days of commits (2026-09-07 → 2026-09-14)
...
identified during this scan. The 7-day window (2026-09-07 → 2026-09-14) contains
**zero commits** on `main` (`git log --since="7 days ago" --pretty=oneline`
returns no output). The most recent commit on `main` is `ff05889 import: finalize
stable release after consumer proof` from 2026-08-05 — a true-merge import of
the reviewed 1.15.1 stable-release ordering fix into the publication repository
(`tokmd`), which was already covered by the 2026-07-27 weekly scan against the
swarm-side commit that prepared the change. Because no new code or configuration
entered the branch during the scan window, the codebase was re-verified against
the existing `.factory/threat-model/threat-model.md` (last modified 2026-08-02,
still within the 90-day freshness window) and all standing defenses (D-01 through
D-23) were re-confirmed. The codebase continues to demonstrate a security-first
design with no regressions.
...
### OBS-006 (carried): Long commit-free interval between weekly scans

| Attribute | Value |
|-----------|-------|
| **Severity** | LOW (process) |
| **STRIDE Category** | N/A — operational |
| **File** | `.github/workflows/droid-security-scan.yml` |
| **Status** | Cadence under review |

**Description:** The current `droid-security-scan` workflow runs weekly
(cron `0 8 * * 1`). This scan (`2026-09-14`) is the first scheduled run
since 2026-07-27, a gap of **49 days**, because no scheduled run was
triggered in August 2026 (likely due to the import stabilization
window). During that interval the `tokmd` publication repository
received the `ff05889` true-merge import (`2026-08-05`) which was
already covered by the swarm-side scan.

**Why not a finding:** Operational continuity, not a code defect. The
import commit itself was reviewed in the swarm workflow prior to
import into `tokmd`. No new code entered the publication repository
during the gap.

**Recommended action (optional):** Verify the cron expression and
manually trigger a scan if the scheduler is misfiring, or update the
schedule to a more frequent cadence if the import cadence warrants it.
...
- **Commits Scanned:** 0 (the 7-day window `git log --since="7 days ago"`
  ...
  2026-08-05, 40 days ago)
- **Files in scope:** Not applicable — no commit diff. The comprehensive
  baseline of 2589 files was last reviewed in the 2026-08-05 import commit
  and was originally fully scanned in the 2026-06-29 weekly scan against
  the same true-merge import that established the swarm branch root.
...
| Subprocess invocation | `crates/tokmd-git/`, `crates/tokmd-cockpit/src/supply_chain.rs`, `crates/tokmd-cockpit/src/gates/contracts.rs`, `crates/tokmd-core/src/context_git/mod.rs`, `crates/tokmd/src/git_support.rs`, `crates/tokmd-scan/src/walk/git.rs` | 0 |
...
| Build / lint | `Cargo.toml`, `deny.toml`, `clippy.toml`, `.cargo/config.toml`, `.cargo/mutants.toml` | 0 |
...
The 7-day window (2026-09-07 → 2026-09-14) contains no commits on `main`
in this repository. The branch root is the `ff05889 import: finalize
stable release after consumer proof` commit, dated 2026-08-05 01:44:26
-0400, which is **40 days** before this scan:
...
The merge's annotated commit message documents it as a history-preserving
import of the 1.15.1 stable-release ordering fix that was reviewed in the
swarm workflow prior to import into the `tokmd` publication repository.
The diff added 2589 files with 1,022,014 insertions. The full content of
this import was already verified by the 2026-07-27 weekly scan against
the swarm-side commit that prepared the change.
