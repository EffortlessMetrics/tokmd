...
| Content I/O helpers | `crates/tokmd-analysis/src/content/io.rs` and `crates/tokmd-analysis/src/content/io/` | coordinator 336; byte owner/tests 108; tag owner/tests 82 | Keep bounded file reads in `io.rs`; keep text detection, BLAKE3 hashing, capped file hashing, and entropy in `io/bytes.rs`; keep tag counting and delimited matching in `io/tags.rs`. Preserve `crate::content::io::*` call sites and the `analysis_content_assets` proof scope. |
