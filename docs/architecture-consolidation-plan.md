...
| Analysis complexity | `crates/tokmd-analysis/src/complexity/mod.rs` + `complexity/functions.rs` + `complexity/details.rs` + `complexity/risk.rs` | 693 + 301 + 343 + 78 | Keep shared complexity logic in `tokmd-analysis`, split language/source helpers |
