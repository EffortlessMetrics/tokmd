...
| Core context/handoff DTO contracts | `crates/tokmd-types/src/lib.rs` and `crates/tokmd-types/src/context.rs` | `lib.rs` 975; context owner 432 | Context receipts, context bundle manifests, handoff manifests, token estimation/audit DTOs, and artifact/capability DTOs now live in a dedicated context owner while root re-exports preserve the public contract |
