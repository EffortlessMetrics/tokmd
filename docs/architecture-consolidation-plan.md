...
| --- | --- | ---: | --- |
| Content complexity | `crates/tokmd-analysis/src/content/complexity.rs` and `crates/tokmd-analysis/src/content/complexity/` | `tests/unit.rs` 995; function test owner 461; production owner modules <=187 | Scoring, nesting, and function-span helpers now live under owner modules; function-detection proof tests now have a dedicated owner; remaining work is mostly cyclomatic/cognitive/nesting test split and aggregation cleanup |
| Content I/O helpers | `crates/tokmd-analysis/src/content/io.rs` and `crates/tokmd-analysis/src/content/io/` | coordinator 351; tag owner/tests 71 | Keep file reads, text detection, hashing, and entropy helpers in `io.rs` for now; keep tag counting and delimited matching in `io/tags.rs`. Preserve `crate::content::io::*` call sites and the `analysis_content_assets` proof scope. |
