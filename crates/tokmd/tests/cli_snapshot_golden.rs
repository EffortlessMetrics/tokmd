...
fn run_tokmd<I, S>(args: I) -> String
where
    ...
    S: AsRef<OsStr>,
{
    let output = tokmd_cmd()
        .args(args)
        .output()
        .expect("failed to execute tokmd command");

    assert!(
        output.status.success(),
        "tokmd command failed with status {}
stderr:
{}",
        output.status,
    ...
    String::from_utf8(output.stdout).expect("tokmd output was not valid UTF-8")
}
