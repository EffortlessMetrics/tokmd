...
fn lang_rows_sorted_by_code_desc_then_name_asc() {
    ...
    for pair in rows.windows(2) {
        let a_code = pair[0]["code"]
            .as_u64()
            .expect("code field must be present and be u64");
        let b_code = pair[1]["code"]
            .as_u64()
            .expect("code field must be present and be u64");
        let a_lang = pair[0]["lang"]
            .as_str()
            .expect("lang field must be present and be a string");
...
fn module_rows_sorted_by_code_desc_then_name_asc() {
    ...
    for pair in rows.windows(2) {
        ...
        let a_mod = pair[0]["module"]
            .as_str()
            .expect("module field must be present and be a string");
...
fn export_rows_sorted_by_code_desc_then_path_asc() {
    ...
    for pair in rows.windows(2) {
        ...
        let a_path = pair[0]["path"]
            .as_str()
            .expect("path field must be present and be a string");
