...
fn lang_rows_sorted_by_code_desc_then_name_asc() {
    ...
    for pair in rows.windows(2) {
        let a_code = pair[0]["code"].as_u64().expect("code must be a valid u64");
        let b_code = pair[1]["code"].as_u64().expect("code must be a valid u64");
        let a_lang = pair[0]["lang"].as_str().expect("lang must be a valid string");
        let b_lang = pair[1]["lang"].as_str().expect("lang must be a valid string");
...
fn module_rows_sorted_by_code_desc_then_name_asc() {
    ...
    for pair in rows.windows(2) {
        let a_code = pair[0]["code"].as_u64().expect("code must be a valid u64");
        let b_code = pair[1]["code"].as_u64().expect("code must be a valid u64");
        let a_mod = pair[0]["module"].as_str().expect("module must be a valid string");
        let b_mod = pair[1]["module"].as_str().expect("module must be a valid string");
...
fn export_rows_sorted_by_code_desc_then_path_asc() {
    ...
    for pair in rows.windows(2) {
        let a_code = pair[0]["code"].as_u64().expect("code must be a valid u64");
        let b_code = pair[1]["code"].as_u64().expect("code must be a valid u64");
        let a_path = pair[0]["path"].as_str().expect("path must be a valid string");
        let b_path = pair[1]["path"].as_str().expect("path must be a valid string");
...
fn export_paths_use_forward_slashes() {
    ...
    for row in rows {
        let path = row["path"].as_str().expect("path must be a valid string");
...
fn export_modules_use_forward_slashes() {
    ...
    for row in rows {
        let module = row["module"].as_str().expect("module must be a valid string");
        assert!(
            !module.contains('\\'),
...
fn lang_totals_match_row_sums() {
    ...
    let sum_code: u64 = rows.iter().map(|r| r["code"].as_u64().expect("code must be a valid u64")).sum();
    let sum_lines: u64 = rows.iter().map(|r| r["lines"].as_u64().expect("lines must be a valid u64")).sum();
    let sum_files: u64 = rows.iter().map(|r| r["files"].as_u64().expect("files must be a valid u64")).sum();

    assert_eq!(sum_code, total["code"].as_u64().expect("total code must be a valid u64"), "code total");
    assert_eq!(sum_lines, total["lines"].as_u64().expect("total lines must be a valid u64"), "lines total");
    assert_eq!(sum_files, total["files"].as_u64().expect("total files must be a valid u64"), "files total");
}
