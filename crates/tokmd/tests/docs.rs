...
fn recipe_context_bundle() {
    ...
    // "tokmd context --budget 128k --strategy spread --mode bundle --output context2.txt"
    let context2_path = tmp.path().join("context2.txt");
    tokmd()
        .arg("context")
        .arg("--budget")
        .arg("128k")
        ...
        .assert()
        .success();
    assert!(context2_path.exists());

    // "tokmd context --budget 128k --mode bundle --compress --output context3.txt"
    let context3_path = tmp.path().join("context3.txt");
    tokmd()
        .arg("context")
        .arg("--budget")
        .arg("128k")
        ...
        .assert()
        .success();
    assert!(context3_path.exists());

    // "tokmd context --budget 128k --module-roots crates,src --strategy spread --output context4.txt"
    let context4_path = tmp.path().join("context4.txt");
    tokmd()
        .arg("context")
        .arg("--budget")
        .arg("128k")
