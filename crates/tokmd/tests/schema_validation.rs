...
/// Returns Ok(None) if the file does not exist (e.g. in some CI environments or packaged builds).
...
fn test_lang_receipt_validates_against_schema() -> Result<()> {
    let Some(schema) = load_schema()? else {
        println!("Skipping schema validation test (schema.json not found)");
        return Ok(());
    };
