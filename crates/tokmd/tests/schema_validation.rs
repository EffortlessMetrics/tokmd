...
/// Returns Ok(None) if the file is missing (e.g. in packaged builds or restricted sandboxes).
fn load_schema() -> Result<Option<Value>> {
    ...
    let schema = serde_json::from_str(&schema_content).context("Failed to parse schema.json")?;
    Ok(Some(schema))
}
...
fn test_lang_receipt_validates_against_schema() -> Result<()> {
    let schema = match load_schema()? {
        Some(s) => s,
        None => {
            eprintln!("Skipping schema validation: docs/schema.json not found");
            return Ok(());
        }
    };
