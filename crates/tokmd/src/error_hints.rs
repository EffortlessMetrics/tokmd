        ...
        || haystack.contains("no such file or directory")
    {
        ...
        if err_msg.starts_with("Path not found: ") {
            ...
            if !path.contains('/') && !path.contains('\\') && !path.contains('.') {
                let subcommands = [
                    "lang", "module", "export", "analyze", "badge", "baseline", "check-ignore",
                    "cockpit", "completions", "context", "diff", "gate", "handoff", "init", "sensor", "tools",
                ];
...
mod tests {
    ...
    fn suggests_did_you_mean_for_misspelled_subcommand() {
        let err = anyhow!("Path not found: anlyze");
        let hints = suggestions(&err);
