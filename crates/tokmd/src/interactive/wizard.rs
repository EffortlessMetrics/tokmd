...
impl ProjectType {
    ...
    pub fn default_module_roots(&self) -> Vec<String> {
        project_type_to_profile(*self).default_module_roots()
    }
}
...
pub fn generate_toml_config(result: &WizardResult) -> Result<String> {
    ...
    let profile = project_type_to_profile(result.project_type);
    let mut config = TomlConfig::default_for_profile(profile);

    // Apply wizard overrides
    config.module.roots = Some(result.module_roots.clone());
    config.module.depth = Some(result.module_depth);
    config.context.budget = Some(result.context_budget.clone());
