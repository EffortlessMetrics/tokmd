...
pub(crate) fn handle(args: cli::CockpitArgs, _global: &cli::GlobalArgs) -> Result<()> {
    ...
    #[cfg(feature = "git")]
    {
        ...
        let receipt = details::compute_cockpit(&repo_root, &args.base, &args.head)?;

        let output = match args.format {
            cli::CockpitFormat::Json => details::render_json(&receipt)?,
            cli::CockpitFormat::Md => details::render_markdown(&receipt),
            cli::CockpitFormat::Sections => details::render_sections(&receipt),
        };
...
mod details {
    ...
    pub fn compute_cockpit(repo_root: &PathBuf, base: &str, head: &str) -> Result<CockpitReceipt> {
    ...
    fn compute_contract_gate(
        ...
        if !contracts_info.api_changed
            && !contracts_info.cli_changed
            && !contracts_info.schema_changed
        {
            return Ok(None);
        }
    ...
    fn generate_review_plan(file_stats: &[FileStats], contracts: &Contracts) -> Vec<ReviewItem> {
        ...
        for fs in file_stats {
            if classify_file(&fs.path) == FileCategory::Test
                && !items.iter().any(|i| i.path == fs.path)
            {
                items.push(ReviewItem {
                    path: fs.path.clone(),
