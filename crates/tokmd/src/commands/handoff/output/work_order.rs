//! Agent work-order rendering for handoff bundles.
...
use super::{HandoffLinkInputs, has_any_link, path_string};
...
pub(in crate::commands::handoff) fn write_work_order(
    out_dir: &Path,
...
fn render_linked_evidence_summary(
    ...
    render_review_packet_check(out, links, &summary.review_packet_check);
    render_review_map(out, links, &summary.review_map);
    render_affected(out, links, &summary.affected);
    render_proof_plan(out, links, &summary.proof_plan);
}

fn render_review_packet_check(
    out: &mut String,
    links: &HandoffLinkInputs<'_>,
    check: &Option<ReviewPacketCheckSummary>,
) {
...
fn render_review_map(
    ...
    if let Some(review_map) = review_map {
        ...
        push_review_map_counts(out, review_map);
        out.push('\n');
...
fn push_review_map_counts(out: &mut String, review_map: &ReviewMapSummary) {
...
fn render_proof_plan(
    ...
    if let Some(proof_plan) = proof_plan {
        out.push_str(&format!(
