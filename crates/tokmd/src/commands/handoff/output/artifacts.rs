//! Core handoff payload and manifest writers.
...
pub(in crate::commands::handoff) struct HandoffPayloads {
    pub(in crate::commands::handoff) map_bytes: u64,
    pub(in crate::commands::handoff) intelligence_bytes: u64,
    pub(in crate::commands::handoff) code_bytes: u64,
    pub(in crate::commands::handoff) artifacts: Vec<ArtifactEntry>,
}

pub(in crate::commands::handoff) fn write_payloads(
    out_dir: &Path,
    export: &ExportData,
...
pub(in crate::commands::handoff) fn write_manifest_json(
    out_dir: &Path,
...
fn write_full_file(
    ...
    if compress {
        bytes += write_non_empty_lines(writer, file_path)?;
    } else {
        bytes += write_raw_content(writer, file_path)?;
    }
...
fn write_non_empty_lines(writer: &mut impl Write, file_path: &Path) -> Result<u64> {
...
fn write_raw_content(writer: &mut impl Write, file_path: &Path) -> Result<u64> {
