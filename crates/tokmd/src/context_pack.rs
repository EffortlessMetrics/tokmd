...
mod tests {
    ...
    fn test_parse_budget() {
        assert_eq!(parse_budget("128k").expect("budget string could not be parsed as a valid token limit"), 128_000);
        assert_eq!(parse_budget("1m").expect("budget string could not be parsed as a valid token limit"), 1_000_000);
        assert_eq!(parse_budget("50000").expect("budget string could not be parsed as a valid token limit"), 50_000);
        assert_eq!(parse_budget("1.5k").expect("budget string could not be parsed as a valid token limit"), 1_500);
    }

    #[test]
    fn test_parse_budget_g_suffix() {
        assert_eq!(parse_budget("1g").expect("budget string could not be parsed as a valid token limit"), 1_000_000_000);
        assert_eq!(parse_budget("0.5g").expect("budget string could not be parsed as a valid token limit"), 500_000_000);
        assert_eq!(parse_budget("2G").expect("budget string could not be parsed as a valid token limit"), 2_000_000_000);
    }

    #[test]
    fn test_parse_budget_unlimited() {
        assert_eq!(parse_budget("unlimited").expect("budget string could not be parsed as a valid token limit"), usize::MAX);
        assert_eq!(parse_budget("max").expect("budget string could not be parsed as a valid token limit"), usize::MAX);
        assert_eq!(parse_budget("UNLIMITED").expect("budget string could not be parsed as a valid token limit"), usize::MAX);
        assert_eq!(parse_budget("MAX").expect("budget string could not be parsed as a valid token limit"), usize::MAX);
        assert_eq!(
            parse_budget("  unlimited  ").expect("budget string could not be parsed as a valid token limit"),
            usize::MAX
        );
    }
    ...
    fn test_parse_budget_with_whitespace() {
        assert_eq!(parse_budget("  10k  ").expect("budget string could not be parsed as a valid token limit"), 10_000);
        assert_eq!(parse_budget(" 5m ").expect("budget string could not be parsed as a valid token limit"), 5_000_000);
    }

    #[test]
    fn test_parse_budget_case_insensitive() {
        assert_eq!(parse_budget("10K").expect("budget string could not be parsed as a valid token limit"), 10_000);
        assert_eq!(parse_budget("2M").expect("budget string could not be parsed as a valid token limit"), 2_000_000);
    }

    #[test]
    fn test_parse_budget_multiplication_k() {
        // Ensure multiplication is correct (not addition or division)
        assert_eq!(parse_budget("2k").expect("budget string could not be parsed as a valid token limit"), 2_000);
        assert_eq!(parse_budget("0.5k").expect("budget string could not be parsed as a valid token limit"), 500);
    }
    ...
    fn test_parse_budget_multiplication_m() {
        // Ensure multiplication is correct (not addition or division)
        assert_eq!(parse_budget("2m").expect("budget string could not be parsed as a valid token limit"), 2_000_000);
        assert_eq!(parse_budget("0.5m").expect("budget string could not be parsed as a valid token limit"), 500_000);
    }
    ...
    fn test_parent_only_invariant_greedy() {
        ...
        for ctx_row in &result {
            let original = rows
                ...
                .expect("selected row should exist in original rows array");
            assert_eq!(
                original.kind,
                FileKind::Parent,
    ...
    fn test_select_files_with_options_spine_reservation() {
        ...
        assert_eq!(readme_entry.expect("README.md should exist in selected files").rank_reason, "spine");
    }
    ...
    fn test_select_files_with_options_rank_reason_hotspot_fallback() {
        ...
        assert!(
            result
                .fallback_reason
                .as_ref()
                .expect("fallback_reason must be Some")
                .contains("hotspot")
        );
    }
    ...
    fn test_assign_policy_over_cap_normal_is_head_tail() {
        ...
        assert!(reason.expect("reason must be Some").contains("head+tail"));
    }
    ...
    fn test_assign_policy_over_cap_generated_is_skip() {
        ...
        assert!(reason.is_some());
        assert!(reason.expect("reason must be Some").contains("generated"));
    }
    ...
    fn test_budget_uses_effective_tokens() {
        ...
        let big = result
            ...
            .expect("big.rs should exist in selected files");
        assert_eq!(big.policy, InclusionPolicy::HeadTail);
        assert!(big.effective_tokens.is_some());
        assert!(big.effective_tokens.expect("effective_tokens must be Some") <= 16_000);
        ...
        let small = result
            ...
            .expect("small.rs should exist in selected files");
