...
fn clean_path(s: &str) -> String {
    ...
    for part in normalized.split('/') {
        if part.is_empty() {
            continue; // ignore duplicate slashes generated by split
        } else if part == ".." {
            if !parts.is_empty() && parts.last() != Some(&"..") {
                parts.pop();
            } else if !is_absolute {
                // Only push '..' if it's not an absolute path root
                parts.push(part);
            }
        } else if part != "." {
            parts.push(part);
...
mod tests {
    ...
    fn test_short_hash_normalizes_parent_directories() {
        assert_eq!(short_hash("src/../src/lib.rs"), short_hash("src/lib.rs"));
        assert_eq!(short_hash("a/b/../../c/d.rs"), short_hash("c/d.rs"));
        assert_eq!(
            short_hash("../../src/lib.rs"),
            short_hash("../../src/lib.rs")
        );
        // Additional edge cases found during review
        assert_eq!(short_hash("foo//../main.rs"), short_hash("main.rs"));
        assert_eq!(short_hash("/../main.rs"), short_hash("/main.rs"));
        assert_eq!(short_hash("."), short_hash("."));
    }

    #[test]
    fn test_redact_path_normalizes_parent_directories() {
        assert_eq!(redact_path("src/../src/lib.rs"), redact_path("src/lib.rs"));
        assert_eq!(redact_path("a/b/../../c/d.rs"), redact_path("c/d.rs"));
    }
}
