//! Deep tests for tokmd-settings TOML loading, serialization, and edge cases.
//!
...
fn paths_with_forward_slashes() -> Result<(), Box<dyn std::error::Error>> {
    let cfg = TomlConfig::parse(
        r#"
[scan]
paths = ["src/main", "crates/tokmd-settings/src"]
exclude = ["**/target/**", "node_modules/"]
"#,
    )?;
    assert_eq!(
        cfg.scan.paths,
        Some(vec!["src/main".into(), "crates/tokmd-settings/src".into()])
    );
...
fn user_config_json_roundtrip() -> Result<(), Box<dyn std::error::Error>> {
    ...
    uc.profiles.insert(
        ...
        Profile {
            ...
            min_code: Some(1),
            max_rows: Some(50),
            redact: Some(tokmd_settings::CliRedactMode::Paths),
            meta: Some(false),
    ...
    assert_eq!(p.redact, Some(tokmd_settings::CliRedactMode::Paths));
...
fn global_args_defaults() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(g.config, tokmd_settings::CliConfigMode::Auto);
    assert!(!g.hidden);
    assert!(!g.no_ignore);
...
fn global_args_to_scan_options_all_fields() -> Result<(), Box<dyn std::error::Error>> {
    let g = GlobalArgs {
        ...
        config: tokmd_settings::CliConfigMode::None,
        hidden: true,
        no_ignore: true,
        no_ignore_parent: true,
        no_ignore_dot: true,
