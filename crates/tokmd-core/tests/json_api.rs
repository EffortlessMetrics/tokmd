...
fn version_data_contains_semver() {
    ...
    let ver = parsed["data"]["version"].as_str().unwrap();
    let parts: Vec<&str> = ver.split('.').collect();
    assert!(
        parts.len() >= 2,
        "version should be semver-like: {ver}"
    );
}
...
fn version_schema_version_is_positive() {
    ...
    let sv = parsed["data"]["schema_version"].as_u64().unwrap();
...
fn lang_mode_receipt_has_tool_info() {
    ...
    assert!(tool.get("version").is_some(), "receipt should have tool.version");
}
...
fn lang_mode_generated_at_ms_is_recent() {
    ...
    let ts = parsed["data"]["generated_at_ms"].as_u64().unwrap();
...
fn lang_mode_with_top_limits_rows() {
    ...
    assert!(rows.len() <= 2, "top=1 should yield at most 2 rows, got {}", rows.len());
}
...
fn lang_mode_default_paths_uses_dot() {
    ...
    let paths = scan["paths"].as_array().unwrap();
...
fn module_mode_with_custom_roots() {
    ...
    let args = &parsed["data"]["args"];
    let roots = args["module_roots"].as_array().unwrap();
...
fn export_mode_success() {
    ...
    assert!(parsed["data"]["rows"].is_array(), "export should have data.rows");
}
...
fn export_mode_with_min_code_filter() {
    ...
    let rows_all = parsed_all["data"]["rows"].as_array().unwrap().len();
...
fn null_values_use_defaults() {
    let result = run_json(
        "lang",
        r#"{"paths": null, "top": null, "files": null}"#,
    );
    let parsed = assert_ok(&result);
...
fn run_json_always_returns_valid_json_for_known_edge_cases() {
    ...
    for (mode, args) in &cases {
        ...
        let parsed: serde_json::Value = serde_json::from_str(&result).unwrap_or_else(|e| {
            panic!(
                "Invalid JSON for mode={mode:?} args={args:?}: {e}\nraw: {result}"
            )
        });
        assert!(
            parsed.get("ok").is_some(),
