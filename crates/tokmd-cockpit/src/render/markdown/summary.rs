...
pub(super) fn append_title(s: &mut String) {
    let _ = writeln!(s, "## Glass Cockpit");
    let _ = writeln!(s);
}

pub(super) fn append_summary_table(s: &mut String, receipt: &CockpitReceipt) {
    s.push_str("### Summary\n\n");
...
pub(super) fn append_summary_comparison(s: &mut String, receipt: &CockpitReceipt) {
