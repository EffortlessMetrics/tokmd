//! Top-level summary and baseline comparison rendering.

use std::fmt::Write;

use crate::{CockpitReceipt, format_signed_f64, trend_direction_label};

pub(super) fn render_header(s: &mut String) {
    let _ = writeln!(s, "## Glass Cockpit");
    let _ = writeln!(s);
}

pub(super) fn render_summary(s: &mut String, receipt: &CockpitReceipt) {
    s.push_str("### Summary\n\n");
...
pub(super) fn render_summary_comparison(s: &mut String, receipt: &CockpitReceipt) {
