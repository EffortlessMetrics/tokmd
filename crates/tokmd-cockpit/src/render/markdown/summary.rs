//! Summary tables for cockpit Markdown receipts.
...
pub(super) fn render(s: &mut String, receipt: &CockpitReceipt) {
    render_current_summary(s, receipt);
    render_comparison_summary(s, receipt);
}

fn render_current_summary(s: &mut String, receipt: &CockpitReceipt) {
    s.push_str("### Summary\n\n");
...
fn render_comparison_summary(s: &mut String, receipt: &CockpitReceipt) {
