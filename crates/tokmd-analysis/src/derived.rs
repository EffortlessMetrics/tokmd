...
pub(crate) fn derive_report(export: &ExportData, window_tokens: Option<usize>) -> DerivedReport {
...
fn build_lang_purity_report(rows: &[&FileRow]) -> LangPurityReport {
    ...
    for row in rows {
        let mod_entry = if let Some(e) = by_module.get_mut(&row.module) {
            e
        } else {
            by_module.entry(row.module.clone()).or_default()
        };

        if let Some(lines) = mod_entry.get_mut(&row.lang) {
            *lines += row.lines;
        } else {
            mod_entry.insert(row.lang.clone(), row.lines);
        }
    }
...
fn build_polyglot_report(rows: &[&FileRow]) -> PolyglotReport {
    ...
    for row in rows {
        if let Some(count) = by_lang.get_mut(&row.lang) {
            *count += row.code;
        } else {
...
pub(crate) fn build_tree(export: &ExportData) -> String {
    #[derive(Default)]
    struct Node {
