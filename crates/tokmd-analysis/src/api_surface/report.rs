//! Stable API surface receipt aggregation.
//!
//! This module owns the deterministic conversion from scanned source symbols
...
pub(crate) fn build_api_surface_report(
    ...
    for rel in files {
        ...
        let text = match crate::content::io::as_text(&bytes) {
            Some(t) => t,
            None => continue,
        };
