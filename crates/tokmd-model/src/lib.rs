...
fn module_key_from_normalized(
    path: &str,
    module_roots: &[String],
    module_depth: usize,
) -> String {
    // path is clean (no \, no leading /, no ./).
    // E.g. "src/main.rs" or "crates/foo/src/lib.rs"
    ...
    // Count remaining parts to determine if 'first' is a directory.
    ...
    // We consumed 'first' (1 segment).
    // Total dirs = 1 + remaining_count.
    // We want min(depth, total_dirs) directories.
    let total_segments = 1 + remaining_count;
    let dirs_available = total_segments - 1; // Exclude filename

    let depth_needed = module_depth.max(1).min(dirs_available);

    // We already have 'first' (depth 1).
    // We need 'depth_needed - 1' more segments from parts.
