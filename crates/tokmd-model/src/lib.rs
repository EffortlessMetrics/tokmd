...
mod mutant_tests {
    ...
    #[test]
    fn env_interpreter_token_basic() {
        let words = vec!["node", "script.js"];
        let token = env_interpreter_token(words.into_iter());
        assert_eq!(token, Some("node"));
    }

    #[test]
    fn env_interpreter_token_empty_and_assignment() {
        let words = vec!["", "VAR=val", "python"];
        let token = env_interpreter_token(words.into_iter());
        assert_eq!(token, Some("python"));
    }

    #[test]
    fn env_interpreter_token_skip_flags() {
        let words = vec!["-S", "--split-string", "-i", "--ignore-environment", "bash"];
        let token = env_interpreter_token(words.into_iter());
        assert_eq!(token, Some("bash"));
    }

    #[test]
    fn env_interpreter_token_skip_next_flags() {
        let words = vec!["-u", "VAR", "--chdir", "/tmp", "ruby"];
        let token = env_interpreter_token(words.into_iter());
        assert_eq!(token, Some("ruby"));
    }

    #[test]
    fn env_interpreter_token_skip_equals_flags() {
        let words = vec!["--unset=VAR", "--chdir=/tmp", "perl"];
        let token = env_interpreter_token(words.into_iter());
        assert_eq!(token, Some("perl"));
    }

    #[test]
    fn env_interpreter_token_skip_other_dashes() {
        let words = vec!["-otherflag", "--unknown-flag", "awk"];
        let token = env_interpreter_token(words.into_iter());
        assert_eq!(token, Some("awk"));
    }

    #[test]
    fn env_interpreter_token_all_skipped() {
        let words = vec!["-S", "-i"];
        let token = env_interpreter_token(words.into_iter());
    ...
    #[test]
    fn test_metrics_from_byte_len() {
        let (bytes, tokens) = metrics_from_byte_len(12);
        assert_eq!(bytes, 12);
        assert_eq!(tokens, 3); // 12 / 4

        let (bytes, tokens) = metrics_from_byte_len(15);
        assert_eq!(bytes, 15);
        assert_eq!(tokens, 3); // 15 / 4

        let (bytes, tokens) = metrics_from_byte_len(0);
        assert_eq!(bytes, 0);
        assert_eq!(tokens, 0);
    }

    #[test]
    fn test_metrics_from_bytes() {
        let data = b"hello world!"; // 12 bytes
        let (bytes, tokens) = metrics_from_bytes(data);
        assert_eq!(bytes, 12);
        assert_eq!(tokens, 3);
    }
}
