...
pub enum Commands {
    ...
    /// Initialize a new project with .tokeignore and optional tokmd.toml.
    Init(InitArgs),
...
impl InitProfile {
    /// Get the default module roots for this profile.
    ...
    /// Get the default module depth for this profile.
    pub fn default_module_depth(&self) -> usize {
        2
    }

    /// Generate the default TOML configuration string for this profile.
    pub fn default_toml(&self) -> Result<String, toml::ser::Error> {
        ...
        toml::to_string_pretty(&config).map(|s| {
            format!(
                "# tokmd configuration\n# Generated by tokmd init --template {:?}\n\n{}",
                self, s
            )
        })
    }
