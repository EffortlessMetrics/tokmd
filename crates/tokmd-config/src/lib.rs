...
pub enum Commands {
    ...
    /// Write a `.tokeignore` template and optionally `tokmd.toml` config.
    Init(InitArgs),
...
impl InitProfile {
    ...
    pub fn default_module_roots(&self) -> Vec<String> {
        match self {
            InitProfile::Rust => vec!["crates".to_string(), "src".to_string()],
            InitProfile::Node => vec![
                "packages".to_string(),
                "apps".to_string(),
                "src".to_string(),
            ],
            InitProfile::Python => vec!["src".to_string(), "lib".to_string()],
            InitProfile::Go => vec!["cmd".to_string(), "pkg".to_string(), "internal".to_string()],
            InitProfile::Cpp => vec!["src".to_string(), "include".to_string(), "lib".to_string()],
            InitProfile::Mono => vec![
                "packages".to_string(),
                "apps".to_string(),
                "libs".to_string(),
            ],
            InitProfile::Default => vec!["src".to_string()],
        }
    }

    /// Get the default module depth for this project type.
    pub fn default_module_depth(&self) -> usize {
        2
    }

    /// Get the default context budget for this project type.
    pub fn default_context_budget(&self) -> String {
        "128k".to_string()
    }
}

impl TomlConfig {
    /// Create a default configuration for a given profile.
    pub fn default_for_profile(profile: InitProfile) -> Self {
        Self {
            module: ModuleConfig {
                roots: Some(profile.default_module_roots()),
                depth: Some(profile.default_module_depth()),
                ..Default::default()
            },
            export: ExportConfig {
            ...
            context: ContextConfig {
                budget: Some(profile.default_context_budget()),
    ...
    /// Serialize configuration to TOML string with header.
