...
fn user_config_json_roundtrip() -> Result<(), Box<dyn std::error::Error>> {
    ...
    uc.profiles.insert(
        ...
        Profile {
            ...
            min_code: Some(1),
            max_rows: Some(50),
            redact: Some(tokmd_cli_args::CliRedactMode::Paths),
            meta: Some(false),
    ...
    assert_eq!(p.redact, Some(tokmd_cli_args::CliRedactMode::Paths));
...
fn global_args_defaults() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(g.config, tokmd_cli_args::CliConfigMode::Auto);
    assert!(!g.hidden);
    assert!(!g.no_ignore);
...
fn global_args_to_scan_options_all_fields() -> Result<(), Box<dyn std::error::Error>> {
    let g = GlobalArgs {
        ...
        config: tokmd_cli_args::CliConfigMode::None,
        hidden: true,
        no_ignore: true,
        no_ignore_parent: true,
        no_ignore_dot: true,
