...
use tokmd_cli_args::{CliLangArgs, GlobalArgs, Profile, TomlConfig, UserConfig, ViewProfile};
...
mod scan_options_conversion {
    ...
    fn config_mode_is_forwarded() {
        use tokmd_cli_args::CliConfigMode;
    ...
    fn lookup_existing_profile() {
        ...
        profiles.insert(
            ...
            Profile {
                ...
                redact: Some(tokmd_cli_args::CliRedactMode::All),
                top: Some(10),
                ..Default::default()
            },
        );
...
mod cli_parsing {
    ...
    fn lang_subcommand_with_format() {
        ...
        match cli.command {
            Some(tokmd_cli_args::Commands::Lang(args)) => {
                assert_eq!(args.format, Some(tokmd_cli_args::CliTableFormat::Json));
            }
    ...
    fn module_subcommand_with_roots_and_depth() {
        ...
        .expect("parse");
        match cli.command {
            Some(tokmd_cli_args::Commands::Module(args)) => {
                assert_eq!(
                    args.module_roots,
    ...
    fn export_subcommand_with_all_options() {
        ...
        .expect("parse");
        match cli.command {
            Some(tokmd_cli_args::Commands::Export(args)) => {
                assert_eq!(args.format, Some(tokmd_cli_args::CliExportFormat::Csv));
                assert_eq!(args.min_code, Some(5));
                assert_eq!(args.max_rows, Some(100));
                assert_eq!(args.redact, Some(tokmd_cli_args::CliRedactMode::Paths));
            }
    ...
    fn analyze_subcommand_with_preset_and_window() {
        let cli =
            Cli::try_parse_from(["tokmd", "analyze", "--preset", "risk", "--window", "200000"])
                .expect("parse");
        match cli.command {
            Some(tokmd_cli_args::Commands::Analyze(args)) => {
                assert_eq!(args.preset, Some(tokmd_cli_args::AnalysisPreset::Risk));
    ...
    fn context_subcommand_defaults() {
        ...
        match cli.command {
            Some(tokmd_cli_args::Commands::Context(args)) => {
    ...
    fn diff_subcommand_with_refs() {
        ...
        match cli.command {
            Some(tokmd_cli_args::Commands::Diff(args)) => {
                assert_eq!(args.refs, vec!["v1.0".to_string(), "v2.0".to_string()]);
                assert_eq!(args.format, tokmd_cli_args::DiffFormat::Md); // default
            }
    ...
    fn gate_subcommand_fail_fast() {
        ...
        match cli.command {
            Some(tokmd_cli_args::Commands::Gate(args)) => {
                assert!(args.fail_fast);
                assert_eq!(args.format, tokmd_cli_args::GateFormat::Text);
            }
    ...
    fn cockpit_subcommand_defaults() {
        ...
        match cli.command {
            Some(tokmd_cli_args::Commands::Cockpit(args)) => {
                assert_eq!(args.base, "main");
                assert_eq!(args.head, "HEAD");
                assert_eq!(args.format, tokmd_cli_args::CockpitFormat::Json);
    ...
    fn handoff_subcommand_defaults() {
        ...
        match cli.command {
            Some(tokmd_cli_args::Commands::Handoff(args)) => {
                assert_eq!(args.budget, "128k");
                assert_eq!(args.preset, tokmd_cli_args::HandoffPreset::Risk);
                assert_eq!(args.rank_by, tokmd_cli_args::ValueMetric::Hotspot);
                assert!(!args.force);
                assert!(!args.compress);
            }
    ...
    fn sensor_subcommand_defaults() {
        ...
        match cli.command {
            Some(tokmd_cli_args::Commands::Sensor(args)) => {
                assert_eq!(args.base, "main");
                assert_eq!(args.head, "HEAD");
                assert_eq!(args.format, tokmd_cli_args::SensorFormat::Json);
            }
    ...
    fn config_mode_none_flag() {
        ...
        assert_eq!(cli.global.config, tokmd_cli_args::CliConfigMode::None);
    }
}
