...
static JS_FUNCTION: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"^\s*(export\s+)?(async\s+)?function\s+\w+").expect("Static regex must compile"));

static JS_ARROW: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"^\s*(export\s+)?(const|let|var)\s+\w+\s*=\s*(async\s+)?\([^)]*\)\s*=>").expect("Static regex must compile")
});

static JS_METHOD: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"^\s*(async\s+)?\w+\s*\([^)]*\)\s*\{").expect("Static regex must compile"));

static GO_FUNC: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"^\s*func\s+\w+").expect("Static regex must compile"));
