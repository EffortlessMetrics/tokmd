...
proptest! {
    ...
    fn lookup_is_deterministic(key in metric_key_strategy()) {
        let r1 = tokmd_analysis_explain::lookup(&key);
        let r2 = tokmd_analysis_explain::lookup(&key);
    ...
    fn lookup_output_is_valid_utf8(key in metric_key_strategy()) {
        if let Some(result) = tokmd_analysis_explain::lookup(&key) {
    ...
    fn catalog_is_valid_utf8_and_nonempty(_dummy in 0..1u8) {
        let cat = tokmd_analysis_explain::catalog();
    ...
    fn catalog_is_deterministic(_dummy in 0..1u8) {
        let c1 = tokmd_analysis_explain::catalog();
        let c2 = tokmd_analysis_explain::catalog();
    ...
    fn canonical_keys_always_resolve(key in metric_key_strategy()) {
        ...
        let _ = tokmd_analysis_explain::lookup(&key);
    }
    ...
    fn normalization_is_consistent(
        ...
        let result = tokmd_analysis_explain::lookup(&normalized_key);
        let canonical_result = tokmd_analysis_explain::lookup(base);
    ...
    fn unknown_keys_return_none(
        ...
        let result = tokmd_analysis_explain::lookup(&key);
    ...
    fn case_insensitive_lookup(key in metric_key_strategy()) {
        let lower = tokmd_analysis_explain::lookup(&key.to_lowercase());
        let upper = tokmd_analysis_explain::lookup(&key.to_uppercase());
