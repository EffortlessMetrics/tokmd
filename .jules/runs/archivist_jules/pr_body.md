## 💡 Summary
Fixed malformed metadata frontmatter in three existing friction items and regenerated the Jules indexes. This ensures the `.jules/index/generated/FRICTION_ROLLUP.md` accurately tracks personas, styles, and shards for open friction rather than listing them as "Unknown", and updates the `RUNS_ROLLUP.md` with missing runs.

## 🎯 Why
The Archivist persona is responsible for consolidating recurring friction themes into better templates and summarizing run packets into generated indexes. Because `librarian_doctest_git_dependency.md`, `steward-release-clean-state.md`, and `fuzz_toolchain_blocker.md` lacked standard `id/persona/style/shard/status` frontmatter, the `cargo xtask jules-index` aggregator could not properly identify them.

## 🔎 Evidence
- files: `.jules/index/generated/FRICTION_ROLLUP.md`
- observed behavior before: friction items showed `Unknown` persona and style.
...
## ✅ Decision
Option A. It's an honest patch that directly improves the Jules scaffolding and indexing health by fixing the root cause of the missing metadata.
...
- Re-formatted `.jules/friction/open/fuzz_toolchain_blocker.md` to include valid frontmatter.
...
```text
{"ts_utc": "2024-05-10T20:55:00Z", "phase": "investigation", "cwd": "/app", "cmd": "cat .jules/index/generated/FRICTION_ROLLUP.md", "status": 0, "summary": "Found that friction items had missing metadata in the rollup."}
{"ts_utc": "2024-05-10T20:56:00Z", "phase": "implementation", "cwd": "/app", "cmd": "python3 fix_friction.py", "status": 0, "summary": "Fixed frontmatter for friction items to match schema."}
{"ts_utc": "2024-05-10T20:57:00Z", "phase": "implementation", "cwd": "/app", "cmd": "cargo xtask jules-index", "status": 0, "summary": "Regenerated the indexes, which correctly updated FRICTION_ROLLUP.md and RUNS_ROLLUP.md"}
```
