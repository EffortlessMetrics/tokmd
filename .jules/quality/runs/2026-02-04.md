...
5. CI failed on Git integration tests because GPG signing was enabled in the environment but no key was available.
...
- Updated `crates/tokmd-git/tests/integration.rs` to explicitly disable GPG signing for temporary test repositories.

### Verification
- `cargo test -p tokmd-config --test determinism`: Passed.
- `cargo test -p tokmd-git --test integration`: Passed.
