...
fn write_or_check_friction_rollup(root: &Path, check: bool) -> Result<()> {
    ...
    let mut items = collect_friction(&friction_dir)?;
    let done_dir = root.join(".jules/friction/done");
    if let Ok(mut done_items) = collect_friction(&done_dir) {
        for item in done_items.iter_mut() {
            item.status = "done".to_string();
        }
        items.append(&mut done_items);
    }
...
fn render_friction_rollup(items: &[FrictionRow]) -> String {
    ...
    out.push_str("It rolls up active friction metadata from `.jules/friction/open/` and `.jules/friction/done/`.\n\n");
