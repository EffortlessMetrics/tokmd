...
pub fn run(_args: JulesIndexArgs) -> Result<()> {
    let root = workspace_root()?;
    write_runs_rollup(&root)?;
    write_friction_rollup(&root)?;
    println!(
        "Jules indexes written under {}",
        root.join(".jules/index/generated").display()
    );
    Ok(())
}

fn write_runs_rollup(root: &Path) -> Result<()> {
    let output_file = root.join(".jules/index/generated/RUNS_ROLLUP.md");
    let runs = collect_runs(root)?;
    ensure_parent(&output_file)?;
    fs::write(&output_file, render_runs_rollup(&runs))
        .with_context(|| format!("write {}", output_file.display()))
}

fn write_friction_rollup(root: &Path) -> Result<()> {
    let output_file = root.join(".jules/index/generated/FRICTION_ROLLUP.md");
    let friction_dir = root.join(".jules/friction/open");
    let items = collect_friction(&friction_dir)?;
    ensure_parent(&output_file)?;
    fs::write(&output_file, render_friction_rollup(&items))
        .with_context(|| format!("write {}", output_file.display()))
}
